...
describe("Presets", function () {
  beforeEach(async function () {
    ...
    // Staking ===============================================================================
    staking = await new Staking__factory(signer).deploy(dao.address, uniswap.address, XXXtoken.address, 10, 100);
  ...
  describe("Staking test", function () {
    ...
    it("stake: Success - Update unfreeze time", async function () {
      await expect(() => staking.stake(10)).changeTokenBalances(uniswap, [staking, signer], [10, -10]);
    ...
    it("stake: Success - Not updating rewards", async function () {
      ...
      await expect(() => staking.stake(1000)).changeTokenBalances(uniswap, [staking, signer], [1000, -1000]);
      expect((await staking.staking(signer.address)).accumulated).to.be.eq(0);
    });
    ...
    it("stake: Success - Updating rewards", async function () {
      ...
      await expect(() => staking.stake(1000)).changeTokenBalances(uniswap, [staking, signer], [1000, -1000]);
      await sleep(100);
      await expect(() => staking.stake(1)).changeTokenBalances(uniswap, [staking, signer], [1, -1]);
  ...
  describe("DAO test", function () {
    beforeEach(async function () {
      ...
      await staking.connect(accs[1]).stake(100);
